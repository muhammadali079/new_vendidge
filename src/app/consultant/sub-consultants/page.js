...
// --- THE FULL 39+ PERMISSION FIELDS CATEGORIZED ---
const PERM_GROUPS = [
  {
    id: "invoices",
    title: "Invoices",
  ...
  },
  {
    ...
    fields: [
      ...
      {
        id: "can_edit_internal_single_unit_price",
        label: "Internal Unit Price",
      },
  ...
  },
  {
    ...
    fields: [
      ...
      {
        id: "can_edit_print_internal_single_unit",
        label: "Print Internal Price",
      },
      { id: "can_edit_print_internal_qty", label: "Print Internal Qty" },
      { id: "can_edit_print_retail_price", label: "Print Retail Price" },
      ...
      { id: "can_edit_print_seller_name", label: "Print Seller Name" },
      { id: "can_edit_print_seller_address", label: "Print Seller Address" },
      { id: "can_edit_print_seller_ntn", label: "Print Seller NTN" },
      { id: "can_edit_print_invoice_date", label: "Print Date" },
      { id: "can_edit_print_challan_no", label: "Print Challan #" },
      { id: "can_edit_print_challan_date", label: "Print Challan Date" },
    ],
  },
];

export default function SubConsultantPage() {
  const [subs, setSubs] = useState([]);
  ...
  // Get current logged-in consultant's perms
  ...
  const [form, setForm] = useState({ name: "", domain_name: "", password: "" });

  const fetchSubs = async () => {
    try {
      console.log("consultant id ", myId);
      console.log("consultant permissions ", myPerms);
      const res = await fetch(
        `/api/consultant/sub-consultant?parentId=${myId}`,
      );
  ...
  const handleOpenModal = (sub = null) => {
    if (sub) {
      ...
      setEditingSub(null);
      // Initialize with 0s for all fields
      const initialPerms = {};
      PERM_GROUPS.forEach((g) =>
        g.fields.forEach((f) => (initialPerms[f.id] = 0)),
      );
      setForm({ name: "", domain_name: "", password: "", ...initialPerms });
    }
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    const res = await fetch("/api/consultant/sub-consultant", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ parent_id: myId, id: editingSub?.id, ...form }),
    });
    if (res.ok) {
      setShowModal(false);
      fetchSubs();
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex justify-between items-end mb-12">
          <div>
            ...
            {parentInfo && (
              <p className="text-indigo-500 font-bold uppercase text-[10px] tracking-widest mt-2 flex items-center gap-2">
                <Shield size={14} /> Agency: {parentInfo.name}
              </p>
            )}
          </div>
          <button
            onClick={() => handleOpenModal()}
            className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black flex items-center gap-2 shadow-xl hover:scale-105 transition-all"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {subs.map((sub) => (
            <div
              key={sub.id}
              className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm hover:shadow-md transition-all relative group"
            >
              <div className="h-14 w-14 bg-slate-100 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                <UserCircle size={32} />
              </div>
              <h3 className="font-black text-xl text-slate-800">{sub.name}</h3>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
                {sub.domain_name}
              </p>
              <button
                onClick={() => handleOpenModal(sub)}
                className="absolute top-8 right-8 p-3 bg-slate-50 rounded-xl hover:bg-indigo-50 hover:text-indigo-600 transition-all"
              >
                <Edit3 size={18} />
              </button>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
          <div
            ...
            onClick={() => setShowModal(false)}
          />
          <div className="relative bg-white w-full max-w-5xl rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
            <div className="p-10 border-b flex justify-between items-center bg-slate-50/50">
              <h2 className="text-3xl font-black text-slate-800">
                {editingSub ? "Edit Agent Access" : "New Agent Access"}
              </h2>
              <button
                onClick={() => setShowModal(false)}
            ...
            <div className="flex-1 overflow-hidden flex">
              {/* Sidebar Tabs */}
              <div className="w-64 bg-slate-50 p-6 border-r flex flex-col gap-2">
                ...
                {PERM_GROUPS.map((g) => (
                  <button
                    ...
                    className={`flex items-center gap-3 px-6 py-4 rounded-2xl font-bold text-xs transition-all ${activeTab === g.id ? "bg-indigo-600 text-white shadow-lg" : "text-slate-500 hover:bg-white"}`}
                  >
                ...
                <div className="space-y-12">
                  <section className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input
                        className="w-full p-5 bg-slate-50 rounded-2xl border-none outline-none font-bold text-slate-700 focus:ring-2 ring-indigo-500/20"
                        value={form.name}
                        onChange={(e) =>
                  ...
                  <section>
                    <h3 className="text-lg font-black text-slate-800 mb-6 flex items-center gap-3">
                      {React.createElement(
                        PERM_GROUPS.find((g) => g.id === activeTab).icon,
                        { size: 24, className: "text-indigo-600" },
                      )}
                      {PERM_GROUPS.find((g) => g.id === activeTab).title} Policy
                    </h3>
                    <div className="grid grid-cols-2 gap-4">
                      {PERM_GROUPS.find((g) => g.id === activeTab).fields.map(
                        (field) => {
                          const isParentAllowed = myPerms[field.id] === 1;
                          return (
                            <div
                              key={field.id}
                              className={`flex items-center justify-between p-6 rounded-[2rem] border transition-all ${isParentAllowed ? "border-slate-100 hover:border-indigo-200" : "bg-slate-50 border-transparent grayscale opacity-50"}`}
                            >
                              ...
                              <label className="relative inline-flex items-center cursor-pointer">
                                ...
                                />
                                <div className="w-12 h-7 bg-slate-200 rounded-full peer peer-checked:after:translate-x-5 peer-checked:bg-indigo-600 after:content-[''] after:absolute after:top-1 after:left-1 after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"></div>
                              </label>
                            </div>
                          );
                        },
            ...
            <div className="p-10 border-t bg-slate-50/50 flex gap-4">
              ...
              <button
                onClick={handleSave}
                className="flex-1 bg-slate-900 text-white py-5 rounded-2xl font-black shadow-2xl flex items-center justify-center gap-3 hover:bg-indigo-700 transition-all"
              >
                <Save size={20} /> SYNC AGENT POLICY
              </button>
            </div>
          </div>
        </div>
